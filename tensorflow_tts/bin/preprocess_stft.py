...
def gen_audio_features(item, config):
    ...
    # downsample audio before stft
    if config["downsample_before_stft"]:
        audio_downsampled = librosa.resample(audio, orig_sr= config["sampling_rate"], target_sr=config['downsampled_sr'])
    ...
    log_mag_stft = np.log10(np.maximum(S, 1e-10)).T  # (#frames, #bins)

    # check audio and feature length
    audio = np.pad(audio, (0, config["fft_size"]), mode="edge")
    audio = audio[: len(log_mag_stft) * hop_size * config['downsample_factor']]
    assert len(log_mag_stft) * hop_size*config['downsample_factor'] == len(audio)

    # extract raw pitch
    _f0, t = pw.dio(
        audio.astype(np.double),
        fs=sampling_rate,
        f0_ceil=sampling_rate//2,
        frame_period=1000 * hop_size / sampling_rate,
    )
    f0 = pw.stonemask(audio.astype(np.double), _f0, t, sampling_rate)
    if len(f0) >= len(log_mag_stft):
        f0 = f0[: len(log_mag_stft)]
    else:
        f0 = np.pad(f0, (0, len(log_mag_stft) - len(f0)))

    # extract energy
    energy = np.sqrt(np.sum(S ** 2, axis=0))
    assert len(log_mag_stft) == len(f0) == len(energy)
    ...
    item["audio"] = audio
    item["log_mag_stft"] = log_mag_stft
    item["f0"] = f0
    item["energy"] = energy
    return True, log_mag_stft, energy, f0, item
...
def save_features_to_file(features, subdir, config):
    ...
    if config["format"] == "npy":
        save_list = [
            ...
            (features["log_mag_stft"], "log_mag_stft-feats", "log_mag_stft-feats", np.float32),
...
def preprocess():
    ...
    build_dir = lambda x: [
        ...
        for y in ["log_mag_stft-feats", "wavs", "ids", "raw-f0", "raw-energies"]
    ]
    build_dir("train")
    build_dir("valid")
    ...
    scaler_log_mag_stft = StandardScaler(copy=False)
    scaler_energy = StandardScaler(copy=False)
    scaler_f0 = StandardScaler(copy=False)

    id_to_remove = []
    for result, log_mag_stft, energy, f0, features in train_map:
        if not result:
        ...
        if len(energy[energy != 0]) == 0 or len(f0[f0 != 0]) == 0:
            id_to_remove.append(features["utt_id"])
            continue
        scaler_log_mag_stft.partial_fit(log_mag_stft)
    ...
    scaler_list = [(scaler_log_mag_stft, ""), (scaler_energy, "_energy"), (scaler_f0, "_f0")]
...
def gen_normal_log_mag_stft(log_mag_stft_path, scaler, config):
    ...
    Args:
        ...
        config (Dict): configuration dictionary.
    """
    log_mag_stft = np.load(log_mag_stft_path)
    log_mag_stft_norm = scaler.transform(log_mag_stft)
    path, file_name = os.path.split(log_mag_stft_path)
    *_, subdir, suffix = path.split(os.sep)
    ...
    np.save(
        ...
        log_mag_stft_norm.astype(np.float32),
        allow_pickle=False,
    )
...
def normalize():
    ...
    if config["format"] == "npy":
        ...
        scaler.n_features_in_ = config["num_bins"]
    else:
        raise ValueError("'npy' is the only supported format.")

    # find all "raw-feats" files in both train and valid folders
    glob_path = os.path.join(config["rootdir"], "**", "log_mag_stft-feats", "*.npy")
    log_mag_stft_raw_feats = glob.glob(glob_path, recursive=True)
    logging.info(f"Files to normalize: {len(log_mag_stft_raw_feats)}")
    ...
    p = Pool(config["n_cpus"])
    partial_fn = partial(gen_normal_log_mag_stft, scaler=scaler, config=config)
    list(p.map(partial_fn, tqdm(log_mag_stft_raw_feats, desc="[Normalizing]")))
...
def compute_statistics():
    ...
    glob_log_mag_stft = glob_fn("log_mag_stft-feats")
    glob_f0 = glob_fn("raw-f0")
    glob_energy = glob_fn("raw-energies")
    assert (
        len(glob_log_mag_stft) == len(glob_f0) == len(glob_energy)
    ), "Features, f0 and energies have different files in training split."

    logging.info(f"Computing statistics for {len(glob_log_mag_stft)} files.")
    # init scaler for multiple features
    scaler_log_mag_stft = StandardScaler(copy=False)
    scaler_energy = StandardScaler(copy=False)
    scaler_f0 = StandardScaler(copy=False)

    for log_mag_stft, f0, energy in tqdm(
        zip(glob_log_mag_stft, glob_f0, glob_energy), total=len(glob_log_mag_stft)
    ):
        # remove outliers
        energy = np.load(energy)
        f0 = np.load(f0)
        # partial fitting of scalers
        scaler_log_mag_stft.partial_fit(np.load(log_mag_stft))
        scaler_energy.partial_fit(energy[energy != 0].reshape(-1, 1))
        scaler_f0.partial_fit(f0[f0 != 0].reshape(-1, 1))
